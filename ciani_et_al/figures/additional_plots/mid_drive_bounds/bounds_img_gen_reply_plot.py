...
freq_ranges = [0.005, 0.015, 0.015]
drive_strs = ["low", "mid", "high"]
min_population = 1e-3
...
with plt.rc_context(TEX_RC_PARAMS):
    fig, axs = plt.subplots(nrows=len(drive_strs), figsize=(6.4, 9.6), dpi=200, sharex=True)
    
    for drive_str, ax, freq_range in zip(drive_strs, axs, freq_ranges):
        data_folder = root_dir / f"{drive_str}_drive_bounds/data"
        da_name = f"{LATTICE_TYPE}_lat_{collision_type}_col_{trans}_{n_photons}-photon_transition_{drive_str}_drive_scan.nc"
        exc_populations = xr.load_dataarray(data_folder / da_name)
    ...
    if SAVE_IMAGE:
        image_name = f"{LATTICE_TYPE}_lat_{collision_type}_col_{trans}_{n_photons}-photon_transition_all_drive_bounds"
        for file_format in ['pdf', 'png']:
            full_name = f"{image_name}.{file_format}"
            plt.savefig(
                IMG_FOLDER / full_name,  
